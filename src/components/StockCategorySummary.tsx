...
export function StockCategorySummary() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setLoading(true);
      try {
        const [stockCategories, stockItems, godownsRes, allInv] = await Promise.all([
        ...
        const summary = stockCategories.map(cat => {
          ...
          if (!selectedGodown) {
            totalQty = catItems.reduce((sum, item) => sum + (item.current_stock || 0), 0);
            totalValue = catItems.reduce((sum, item) => sum + ((item.current_stock || 0) * (item.avg_cost || item.opening_rate || 0)), 0);
          } else {
              ...
              >
                ...
                <div className="space-y-3 mb-6 flex-1 italic text-xs uppercase tracking-widest border-t border-border pt-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-foreground">{formatNumber(cat.totalQty)}</span>
                  </div>
                  <div className="flex justify-between items-center">
