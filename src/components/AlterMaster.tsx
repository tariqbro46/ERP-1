...
export const AlterMaster: React.FC = () => {
  ...
  const renderModern = () => (
    <div className="flex flex-col min-h-full bg-slate-50/50 transition-colors">
      ...
      <div className="sticky top-0 z-[40] bg-white/90 backdrop-blur-md border-b border-slate-100 shadow-sm p-5 lg:p-6 space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              {selectedCategory ? (
                ...
                <EditableHeader 
                  pageId="alter_master_modern"
                  defaultTitle={t('common.alteration')}
                  defaultSubtitle="Premium Master Configuration Hub"
                />
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {!selectedCategory && (
              <div className="bg-slate-100 rounded-lg p-1 hidden md:flex items-center gap-1.5 text-xs text-slate-500 font-medium">
                <LayoutGrid className="w-3.5 h-3.5 ml-1.5" />
                <span className="mr-1.5">Total: {MASTER_CATEGORIES.length} Modules</span>
              </div>
            )}
            {selectedCategory && (
              <motion.button
        ...
        {/* Dynamic Controls Row (Filters and Search) */}
        <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center">
          {!selectedCategory ? (
            <div className="flex items-center bg-slate-100/80 p-1 rounded-xl self-start overflow-x-auto max-w-full">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder={selectedCategory ? `Search ${t(selectedCategory.labelKey)}...` : "Quick search master categories..."}
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
